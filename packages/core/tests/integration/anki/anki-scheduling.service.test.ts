...
describe("AnkiSchedulingService", () => {
	...
	describe("convert - routing", () => {
		it("uses replayScheduling when revlogs present", () => {
			...
			const revlogs = [createAnkiRevlog({ id: 1700000000000, ease: 3 })];
	...
	describe("replayScheduling", () => {
		...
		it("sorts revlog by id before replaying", () => {
			...
			expect(resultReversed.difficulty).toBeCloseTo(resultSorted.difficulty, 5);
		});

		it("clamps ease 0 to 1 (Again)", () => {
			const ankiCard = createAnkiCard();
			const revlogsAgain = [createAnkiRevlog({ id: 1700000001000, ease: 0 })];
			const revlogsGood = [createAnkiRevlog({ id: 1700000001000, ease: 3 })];
		...
		it("clamps ease 5 to 4 (Easy)", () => {
			const ankiCard = createAnkiCard();
			const revlogs = [createAnkiRevlog({ id: 1700000001000, ease: 5 })];

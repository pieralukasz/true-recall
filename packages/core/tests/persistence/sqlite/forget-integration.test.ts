...
describe("Forget Integration", () => {
	...
	describe("forget updates countByState badge counts", () => {
		it("review-state card forgotten → new count increases", () => {
			...
			counts = countByState([newCard1, newCard2, forgotten], reviewedToday, 4);
		...
		it("all scheduling fields reset to initial values", () => {
			const card = createMockFlashcard({
				...
				fsrs: {
					...
					suspended: true,
					buriedUntil: new Date(Date.now() + 86400000).toISOString(),
				},
			});
	...
	describe("edge cases", () => {
		...
		it("forget all cards from a note → all become New and available", () => {
			...
			const reviewedToday = new Set(cards.slice(0, 3).map((c) => c.id));
			...
			const queue = reviewService.buildQueue(forgottenCards, fsrsService, {
				...defaultOptions,
				reviewedToday,
			});

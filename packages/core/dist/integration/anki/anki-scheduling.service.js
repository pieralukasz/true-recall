...
export class AnkiSchedulingService {
    constructor(fsrsService) {
        this.fsrsService = fsrsService;
    }
    replayScheduling(cardId, ankiCard, revlogs) {
    ...
    mapSchedulingDirect(cardId, ankiCard) {
        const now = new Date();
        ...
        if (ankiCard.factor > 0) {
            ...
            card.difficulty = Math.max(DIFFICULTY_MIN, Math.min(DIFFICULTY_MAX, DIFFICULTY_INVERSION_CONSTANT - ease));
        }
        if (state === State.New) {
    ...
    convert(cardId, ankiCard, revlogs) {
        if (revlogs.length > 0) {
    ...
    applyStatus(card, ankiCard) {
        if (ankiCard.queue === ANKI_QUEUE_SUSPENDED) {
            card.suspended = true;
        }
        if (ankiCard.queue === ANKI_QUEUE_BURIED_USER ||

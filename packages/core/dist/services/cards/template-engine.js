...
export function renderTemplate(template, context) {
    ...
    // 3. Replace {{cloze:FieldName}}
    working = working.replace(/\{\{\s*cloze:([\w][\w ]*?)\s*\}\}/g, (_match, fieldName) => {
        var _a;
        const fieldValue = context.fields[fieldName];
        if (fieldValue === undefined)
            return _match;
        const idx = (_a = context.clozeIndex) !== null && _a !== void 0 ? _a : 0;
    ...
    working = working.replace(/\{\{\s*([\w][\w ]*?)\s*\}\}/g, (_match, fieldName) => {
...
function processConditionals(template, fields) {
    ...
    while (changed && iterations < MAX_ITERATIONS) {
        ...
        result = result.replace(/\{\{([#^])\s*([\w][\w ]*?)\s*\}\}((?:(?!\{\{[#^])[\s\S])*?)\{\{\/\s*([\w][\w ]*?)\s*\}\}/g, (_match, type, openField, content, closeField) => {
            var _a;
            changed = true;
            ...
            const fieldValue = (_a = fields[openField]) !== null && _a !== void 0 ? _a : "";
 ...
 */
function renderClozeNested(text, targetIndex, isAnswer) {
    let result = "";
    let i = 0;
    while (i < text.length) {
        ...
        if (text.startsWith("{{c", i)) {
            ...
            if (parsed) {
                if (parsed.index === targetIndex) {
                    if (isAnswer) {
                        const inner = renderClozeNested(parsed.content, targetIndex, isAnswer);
                        result += `**${inner}**`;
                    }
                    else {
 ...
 */
function parseClozeAt(text, start) {
    // Must start with {{c

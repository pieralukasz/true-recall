export class IntegrityCheckService {
    ...
    check() {
        ...
        const orphanedReviewLogs = this.db
            .query(`SELECT rl.id FROM review_log rl
        ...
        return {
            ...
            orphanedNotes,
            orphanedReviewLogs,
            totalIssues: orphanedCards.length + orphanedNotes.length + orphanedReviewLogs.length,
        };
    }
    repair(report) {
        const now = Date.now();
     ...
     */
    checkAndRepairOnce() {
        const row = this.db.get(`SELECT value FROM meta WHERE key = 'integrity_checked'`);
        if ((row === null || row === void 0 ? void 0 : row.value) === "1")
            return 0;
        const report = this.check();
        let fixed = 0;
        if (report.totalIssues > 0) {
            fixed = this.repair(report);
            console.debug(`[True Recall] Integrity check: fixed ${fixed} orphaned records`);
        }
        this.db.run(`INSERT OR REPLACE INTO meta (key, value) VALUES ('integrity_checked', '1')`);
        return fixed;
    }
}

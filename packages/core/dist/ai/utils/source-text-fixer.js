const MD_INLINE_RE = /\*{1,3}|_{1,3}|~~|==|`{1,3}|\[([^\]]*)\]\([^)]*\)|!\[([^\]]*)\]\([^)]*\)/g;
...
function buildPositionMap(text) {
    ...
    while (i < len) {
        ...
        if (ch === "*" || ch === "_" || ch === "~" || ch === "=") {
            let end = i + 1;
            while (end < len && charAt(text, end) === ch && end - i < 3)
                end++;
            if ((ch === "~" && end - i === 2) ||
                (ch === "=" && end - i === 2) ||
                ch === "*" ||
...
export function fixSourceText(sourceText, inputText) {
    if (!sourceText)
        return undefined;
    // 1. Exact match — ideal case
    if (inputText.includes(sourceText))
        return sourceText;
    // 2. Try with trimmed whitespace variants
    const trimmed = sourceText.trim();
    if (trimmed !== sourceText && inputText.includes(trimmed))
        return trimmed;
    ...
    if (origStart === undefined || origEnd === undefined)
        return undefined;
    const isMarkdownChar = (ch) => ch === "*" || ch === "_" || ch === "~" || ch === "=" || ch === "`";
...
export function fixBlockSourceTexts(blocks, inputText) {

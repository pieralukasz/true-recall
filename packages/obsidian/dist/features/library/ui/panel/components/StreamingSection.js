...
export function StreamingSection({ currentFilePath, }) {
    ...
    const isActive = streaming.isGenerating && streaming.notePath === currentFilePath;
    ...
    useEffect(() => {
        ...
        if (elapsed >= SCROLL_THROTTLE_MS) {
            ...
            scrollTimerRef.current = setTimeout(scrollToEnd, SCROLL_THROTTLE_MS - elapsed);
        }
        return () => clearTimeout(scrollTimerRef.current);
    });
    if (!isActive)
        return null;
    return (_jsxs(_Fragment, { children: [_jsx(PartialCard, { streaming: streaming }), _jsx("div", { ref: sentinelRef })] }));
}
export function useStreamingCardState() {
    ...
    useSignalEffect(() => {
        ...
        if (prev.isGenerating !== s.isGenerating ||

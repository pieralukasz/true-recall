...
export function useStreamingText(fullText) {
    ...
    const newTimestampsRef = useRef(new Map());
    ...
    const reducedMotion = useRef(typeof window !== "undefined" &&
    ...
    if (currentVisible !== lastVisibleRef.current ||
        ...
        cachedWordsRef.current = allWords
            ...
            return ({
                text,
                isNew: newTimestampsRef.current.has(i) &&
                    now - ((_a = newTimestampsRef.current.get(i)) !== null && _a !== void 0 ? _a : 0) < NEW_WORD_DURATION_MS,
            });
        });

...
function createInitialState() {
    return {
        ...
        editMode: Object.assign({}, INITIAL_EDIT_MODE),
    };
}
function getBadgeTypeForState(cardState) {
...
function computeBadgeCounts(queue, startIndex) {
...
export function createReviewSlice(set, get, _deps) {
    ...
    const slice = Object.assign(Object.assign({}, initial), { startSession: (queue) => {
            const cachedBadgeCounts = computeBadgeCounts(queue, 0);
            schedulingPreview = null;
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), { isActive: true, queue: [...queue], currentIndex: 0, isAnswerRevealed: false, results: [], startTime: Date.now(), questionShownTime: Date.now(), stats: {
                        total: queue.length,
                        reviewed: 0,
                        again: 0,
                        hard: 0,
            ...
            schedulingPreview = null;
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), { isActive: false, editMode: Object.assign({}, INITIAL_EDIT_MODE), stats: Object.assign(Object.assign({}, s.review.stats), { duration: Date.now() - s.review.startTime }) }),
            }));
        }, reset: () => {
            schedulingPreview = null;
            const initialState = createInitialState();
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), initialState),
            }));
        }, revealAnswer: () => {
            const state = get().review;
            if (!state.isActive || state.isAnswerRevealed)
                return;
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), { isAnswerRevealed: true }),
            }));
        }, hideAnswer: () => {
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), { isAnswerRevealed: false }),
            }));
        }, nextCard: () => {
            const state = get().review;
            if (!state.isActive)
            ...
            const newCounts = Object.assign({}, state.cachedBadgeCounts);
            if (currentCard) {
            ...
            schedulingPreview = null;
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), { currentIndex: nextIndex, isAnswerRevealed: false, questionShownTime: Date.now(), cachedBadgeCounts: newCounts }),
            }));
            return nextIndex < state.queue.length;
        }, recordAnswer: (rating, updatedCard) => {
            const state = get().review;
            if (!state.isActive)
                return false;
            const currentCard = state.queue[state.currentIndex];
            ...
            const result = {
                ...
                elapsedDays: currentCard.fsrs.lastReview
                    ? Math.floor((Date.now() - new Date(currentCard.fsrs.lastReview).getTime()) /
                        (1000 * 60 * 60 * 24))
                    : 0,
            };
            ...
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), { queue: newQueue, results: [...s.review.results, result] }),
            }));
            return true;
        }, recordAnswerAndNext: (rating, updatedCard, requeueData) => {
            const state = get().review;
            if (!state.isActive)
                return false;
            const currentCard = state.queue[state.currentIndex];
            if (!currentCard)
                return false;
            // Update badge counts incrementally - O(1)
            const newBadgeCounts = Object.assign({}, state.cachedBadgeCounts);
            ...
            if (nextIndex < newQueue.length) {
                ...
                if (nextCard) {
                    const nextIsLearning = nextCard.fsrs.state === State.Learning ||
                    ...
                    if (nextIsLearning && !isCardDueNowInternal(nextCard)) {
                        for (let i = nextIndex + 1; i < newQueue.length; i++) {
                            ...
                            if (!candidate)
                                continue;
                            const candidateIsLearning = candidate.fsrs.state === State.Learning ||
            ...
            schedulingPreview = null;
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), { queue: newQueue, results: [...s.review.results, result], currentIndex: nextIndex, isAnswerRevealed: false, questionShownTime: Date.now(), cachedBadgeCounts: newBadgeCounts }),
            }));
            return nextIndex < newQueue.length;
        }, requeueCard: (card, position) => {
            const state = get().review;
            const newQueue = [...state.queue];
            ...
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), { queue: newQueue, cachedBadgeCounts: newCounts }),
            }));
        }, removeCurrentCard: () => {
            const state = get().review;
            if (!state.isActive)
            ...
            schedulingPreview = null;
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), { queue: newQueue, isAnswerRevealed: false, questionShownTime: Date.now(), cachedBadgeCounts: newCounts }),
            }));
            ...
            else if (cardIndex === state.currentIndex &&
                newIndex >= newQueue.length) {
                newIndex = Math.max(0, newQueue.length - 1);
            }
            schedulingPreview = null;
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), { queue: newQueue, currentIndex: newIndex, isAnswerRevealed: false, questionShownTime: Date.now(), cachedBadgeCounts: newCounts }),
            }));
        }, removeCardsByIds: (cardIds) => {
            const state = get().review;
            if (!state.isActive || cardIds.length === 0)
                return;
            ...
            if (!state.isActive)
                return;
            if (state.queue.some((c) => c.id === card.id))
                return;
            const badgeType = getBadgeTypeForState(card.fsrs.state);
            const newCounts = Object.assign({}, state.cachedBadgeCounts);
            newCounts[badgeType]++;
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), { queue: [...s.review.queue, card], cachedBadgeCounts: newCounts }),
            }));
        }, insertCardAtPosition: (card, position) => {
            const state = get().review;
            if (!state.isActive)
                return;
            const clampedPosition = Math.max(0, Math.min(position, state.queue.length));
            const newCounts = Object.assign({}, state.cachedBadgeCounts);
        ...
        }, undoLastAnswer: (previousIndex, restoredCard, requeuedAtIndex) => {
            const state = get().review;
            if (!state.isActive)
                return;
            const newCounts = Object.assign({}, state.cachedBadgeCounts);
            ...
            if (requeuedAtIndex !== undefined &&
                requeuedAtIndex < state.queue.length) {
                const requeuedCard = state.queue[requeuedAtIndex];
                if (requeuedCard) {
                    const requeuedBadgeType = getBadgeTypeForState(requeuedCard.fsrs.state);
            ...
            schedulingPreview = null;
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), { queue: newQueue, currentIndex: previousIndex, isAnswerRevealed: false, questionShownTime: Date.now(), results: newResults, cachedBadgeCounts: newCounts }),
            }));
        }, getEditState: () => (Object.assign({}, get().review.editMode)), startEdit: (field) => {
            const state = get().review;
            const card = state.queue[state.currentIndex];
            if (!card)
                return;
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), { editMode: {
                        active: true,
                        field,
            ...
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), { editMode: Object.assign({}, INITIAL_EDIT_MODE) }),
            }));
        }, isEditing: () => get().review.editMode.active, updateCurrentCardContent: (question, answer) => {
            const state = get().review;
            const card = state.queue[state.currentIndex];
            if (!card)
                return;
            const newQueue = [...state.queue];
            newQueue[state.currentIndex] = Object.assign(Object.assign({}, card), { question,
                answer });
            set((s) => ({
                review: Object.assign(Object.assign({}, s.review), { queue: newQueue }),
            }));
        }, getSchedulingPreview: () => schedulingPreview, setSchedulingPreview: (preview) => {
            schedulingPreview = preview;
        }, notifyChange: () => {
            set((s) => ({ review: Object.assign({}, s.review) }));
        }, getCurrentCard: () => {
            var _a;
            const state = get().review;
            ...
            return (_a = state.queue[state.currentIndex]) !== null && _a !== void 0 ? _a : null;
        }, getPhase: () => {
            const state = get().review;
            if (!state.isActive) {
            ...
            const currentCard = state.queue[state.currentIndex];
            if (currentCard) {
                const isLearning = currentCard.fsrs.state === State.Learning ||
            ...
            return { type: "idle" };
        }, getBadgeCounts: () => (Object.assign({}, get().review.cachedBadgeCounts)), getStats: () => {
            const state = get().review;
            ...
            return {
                ...
                learningCards: results.filter((r) => r.previousState === State.Learning ||
                    r.previousState === State.Relearning).length,
        ...
        }, isCardDueNow: isCardDueNowInternal, getPendingLearningCards: () => {
            const state = get().review;
            ...
            if (!isLearning)
                return false;
            if (isCardDueNowInternal(currentCard))
                return false;
        ...
        }, isActiveSession: () => get().review.isActive, isAnswerShown: () => get().review.isAnswerRevealed });
    return slice;
}

...
export function registerEventHandlers(plugin) {
        ...
        if (file instanceof TFile && file.extension === "md") {
            ...
            menu.addItem((item) => {
                item
                    ...
                    var _a, _b;
                    const { NamePromptModal } = yield import("@true-recall/obsidian/modals/study/NamePromptModal");
                    const modal = new NamePromptModal(plugin.app, file.basename);
                    const result = yield modal.openAndWait();
                    if (result.cancelled)
                        return;
                    const name = result.name;
                    const folderPath = (_b = (_a = file.parent) === null || _a === void 0 ? void 0 : _a.path) !== null && _b !== void 0 ? _b : "";
                    const projectPath = normalizePath(folderPath ? `${folderPath}/${name}.md` : `${name}.md`);
            ...
            if (!plugin.app.vault.getAbstractFileByPath(notePath)) {
                menu.addItem((item) => {
                    item
                        ...
                        yield plugin.app.vault.create(notePath, content);
                        yield plugin.app.workspace.openLinkText(notePath, "", false);
                    }));
                });
            }
        }
...
function updatePanelView(plugin, file) {
    const { workspace } = plugin.app;
    const activeLeaf = workspace.activeLeaf;
    const activeView = workspace.getActiveViewOfType(ItemView);
    const isReviewViewActive = (activeView === null || activeView === void 0 ? void 0 : activeView.getViewType()) === VIEW_TYPE_REVIEW;
    const isPanelActive = (activeView === null || activeView === void 0 ? void 0 : activeView.getViewType()) === VIEW_TYPE_FLASHCARD_PANEL;
    ...
    const isMainArea = (activeLeaf === null || activeLeaf === void 0 ? void 0 : activeLeaf.getContainer()) === workspace.rootSplit;
    ...
    leaves.forEach((leaf) => {
        ...
        if (view instanceof FlashcardPanelView) {
            ...
            if (isReviewViewActive) {
                const review = (_b = (_a = plugin.store) === null || _a === void 0 ? void 0 : _a.getState()) === null || _b === void 0 ? void 0 : _b.review;
                if (review === null || review === void 0 ? void 0 : review.isActive) {
 ...
 */
export function registerDeletionHandler(plugin, deletionHandler) {

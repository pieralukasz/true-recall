...
export function registerDashboardCodeblocks(plugin) {
    // ── Existing widgets ────────────────────────────────────────
    plugin.registerMarkdownCodeBlockProcessor("true-recall-dashboard", (_source, el, _ctx) => {
    ...
    plugin.registerMarkdownCodeBlockProcessor("true-recall-note-stats", (_source, el, ctx) => {
    ...
    plugin.registerMarkdownCodeBlockProcessor("true-recall-unassigned", (_source, el, _ctx) => {
        el.addClass("true-recall-codeblock-unassigned");
        const unmount = mountPreact(el, plugin, h(UnassignedNotesWidget, null));
        registerCleanup(el, unmount);
    });
    plugin.registerMarkdownCodeBlockProcessor("true-recall-project-hub", (_source, el, _ctx) => {
        el.addClass("true-recall-codeblock-project-hub");
        const unmount = mountPreact(el, plugin, h(ProjectHubWidget, null));
        registerCleanup(el, unmount);
    });
    // ── New global widgets ──────────────────────────────────────
    plugin.registerMarkdownCodeBlockProcessor("true-recall-progress", (source, el, _ctx) => {
        el.addClass("true-recall-codeblock-progress");
        const unmount = mountPreact(el, plugin, h(ProgressWidget, { source }));
        registerCleanup(el, unmount);
    });
    plugin.registerMarkdownCodeBlockProcessor("true-recall-achievements", (source, el, _ctx) => {
        el.addClass("true-recall-codeblock-achievements");
        const unmount = mountPreact(el, plugin, h(AchievementsWidget, { source }));
        registerCleanup(el, unmount);
    });
    plugin.registerMarkdownCodeBlockProcessor("true-recall-answer-streak", (source, el, _ctx) => {
        el.addClass("true-recall-codeblock-answer-streak");
        const unmount = mountPreact(el, plugin, h(AnswerStreakWidget, { source }));
        registerCleanup(el, unmount);
    });
    plugin.registerMarkdownCodeBlockProcessor("true-recall-countdown", (source, el, _ctx) => {
        el.addClass("true-recall-codeblock-countdown");
        const unmount = mountPreact(el, plugin, h(CountdownWidget, { source }));
        registerCleanup(el, unmount);
    });
    plugin.registerMarkdownCodeBlockProcessor("true-recall-maturity", (source, el, _ctx) => {
        el.addClass("true-recall-codeblock-maturity");
        const unmount = mountPreact(el, plugin, h(MaturityWidget, { source }));
        registerCleanup(el, unmount);
    });
    plugin.registerMarkdownCodeBlockProcessor("true-recall-ratings", (source, el, _ctx) => {
...
function resolveSourceUid(plugin, sourcePath) {
    var _a;
    const file = plugin.app.vault.getAbstractFileByPath(sourcePath);
    if (!file)
        return null;
    const uids = plugin.frontmatterIndex.getValues("flashcard_uid", sourcePath);
    return (_a = uids[0]) !== null && _a !== void 0 ? _a : null;
}
function registerCleanup(el, unmount) {
    var _a;
    ...
    observer.observe((_a = el.parentElement) !== null && _a !== void 0 ? _a : document.body, {
        childList: true,
        subtree: true,
    });
}

...
import { buildSampleFields, previewNoteTypeCss } from "./template-preview";

interface TemplatePreviewProps {
	template: CardTemplate;
	fields: string[];
	noteTypeType: 0 | 1;
	/** Note type Styling to apply (the text being edited, when live) */
	css?: string;
	noteTypeId?: string;
}
...
export function TemplatePreview({
	...
	const sampleFields = useMemo(
		() => buildSampleFields(fields, noteTypeType),
		[fields, noteTypeType],
	);

	const noteTypeCss = useMemo(
		() => previewNoteTypeCss(css, noteTypeId, template.ordinal, noteTypeType),
		[css, noteTypeId, template.ordinal, noteTypeType],
	);
			...
			{showPreview && (
				<div class="ep:mt-2 ep:border ep:border-obs-border ep:rounded-md ep:p-3 ep:bg-obs-primary/50">
					...
					<MarkdownContent
						...
						class={cn("ep:mb-3 ep:text-ui-small", noteTypeCss?.className)}
					/>
					...
					<MarkdownContent
						...
						class={cn("ep:text-ui-small", noteTypeCss?.className)}
					/>
				</div>
			)}
		</div>

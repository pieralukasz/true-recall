import { DEFAULT_LMSTUDIO_BASE_URL, type TrueRecallSettings } from "@true-recall/core";
import {
	Clickable,
	FormField,
	InfoBlock,
...
export function LMStudioScopedModelField({
	...
	return (
		<FormField name={name} description={description}>
			{lmState.status === "loading" && <InfoBlock>Discovering models…</InfoBlock>}
			{lmState.status === "error" && (
				<>
					...
					<TextInput
						value={value}
						onChange={(v) => void save({ [modelKey]: v } as Partial<TrueRecallSettings>)}
			...
			{lmState.status === "ready" && lmState.models.length === 0 && (
				<>
					<InfoBlock>No models found — load a model in LM Studio first.</InfoBlock>
					<TextInput
						value={value}
						onChange={(v) => void save({ [modelKey]: v } as Partial<TrueRecallSettings>)}
						placeholder="e.g. llama-3.2-3b-instruct"

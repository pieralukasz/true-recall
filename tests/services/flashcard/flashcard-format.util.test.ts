...
describe("previewCards", () => {
	describe("basic cards", () => {
		it("returns single preview for basic card", () => {
			const previews = previewCards("What is X? :: Definition");
		...
		it("returns empty for input without :: separator", () => {
	...
	describe("cloze cards", () => {
		...
		it("handles cloze with hints", () => {
			const content =
				"{{c1::Paris::capital}} is in {{c2::France::country}} :: Europe";
			const previews = previewCards(content);
		...
		it("handles single cloze deletion", () => {
			const content = "The answer is {{c1::42}} :: Math fact";
			const previews = previewCards(content);
		...
		it("handles 3 cloze indices", () => {
			const content = "{{c1::A}} {{c2::B}} {{c3::C}} :: Extra";
			const previews = previewCards(content);
	...
	describe("mixed content", () => {
		it("handles basic + cloze together", () => {
			const content = [
				"Simple Q :: Simple A",
				"{{c1::Tokyo}} in {{c2::Japan}} :: Geography",
			].join("\n");
			const previews = previewCards(content);
	...
	describe("edge cases", () => {
		it("handles unicode content", () => {
			const content = "こんにちは means? :: Hello in Japanese";
			const previews = previewCards(content);
		...
		it("handles wiki-links in content", () => {
			const content = "What is [[Obsidian]]? :: A note app";
			const previews = previewCards(content);
...
describe("preview and serialize consistency", () => {
	...
	it("preview labels match card types", () => {
		...
		expect(labels).toEqual(["Basic", "Cloze 1", "Cloze 2"]);
	});
});

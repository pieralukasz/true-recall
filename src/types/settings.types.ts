...
export function extractFSRSSettings(settings: TrueRecallSettings): FSRSSettings {
    const defaultPreset = settings.fsrsPresets?.find(p => p.id === settings.defaultPresetId);
    if (defaultPreset) {
...
export function extractFSRSSettingsFromPreset(preset: FSRSPreset): FSRSSettings {
    return {

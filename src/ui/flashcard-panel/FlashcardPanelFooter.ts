	...
	private renderNormalFooter(): void {
		...
		if (hasUncollectedFlashcards && onCollect) {
			const collectBtn = buttonsWrapper.createEl("button", {
				cls: "ep-bg-collect-gradient ep:flex-1 ep:border-none ep:py-2.5 ep:px-4 ep:rounded-md ep:cursor-pointer ep:font-semibold ep:text-ui-small ep:transition-colors ep:text-obs-normal",
			});

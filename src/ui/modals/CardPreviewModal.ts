	...
	protected renderBody(container: HTMLElement): void {
		...
		if (this.options.category === "buried" && this.options.cards.length > 0) {
			const unburyAllBtn = headerEl.createEl("button", {
				cls: "ep:text-ui-smaller ep:py-1.5 ep:px-3 ep:bg-obs-interactive ep:text-obs-on-accent ep:border-none ep:rounded-md ep:cursor-pointer ep:transition-colors ep:hover:opacity-90",
				text: "Unbury all",
			});
		...
		if (this.options.category === "suspended" && this.options.cards.length > 0) {
			const deleteAllBtn = headerEl.createEl("button", {
				cls: "ep:text-ui-smaller ep:py-1.5 ep:px-3 ep:bg-obs-red ep:text-obs-on-accent ep:border-none ep:rounded-md ep:cursor-pointer ep:transition-colors ep:hover:opacity-90",
				text: "Delete all",
			});

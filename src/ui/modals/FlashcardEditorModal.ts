...
export class FlashcardEditorModal extends BaseModal {
	...
	private renderButtons(container: HTMLElement): void {
		...
		this.saveButton = this.buttonsContainer.createEl("button", {
			text: buttonText,
			cls: "ep:py-3 ep:px-5 ep:bg-obs-interactive ep:text-obs-on-accent ep:border-none ep:rounded-md ep:cursor-pointer ep:font-medium ep:transition-colors ep:hover:bg-obs-interactive-hover ep:disabled:opacity-50 ep:disabled:cursor-not-allowed",
		});

...
export class EasyDaysModal extends BasePromiseModal<EasyDaysResult> {
	...
	private renderSpecificDatesSection(container: HTMLElement): void {
		...
		const addBtn = inputRow.createEl("button", {
			text: "+ add",
			type: "button",
			cls: "ep:px-4 ep:py-2 ep:rounded-md ep:bg-obs-interactive ep:text-obs-on-accent ep:border-none ep:text-ui-small ep:font-medium ep:cursor-pointer ep:hover:opacity-90",
		});

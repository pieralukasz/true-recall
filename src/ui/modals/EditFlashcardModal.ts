 ...
 * Allows user to edit an existing flashcard's question and answer
...
export interface EditFlashcardResult {
	cancelled: boolean;
	question: string;
	answer: string;
}

export interface EditFlashcardModalOptions {
 ...
 */
export class EditFlashcardModal extends BaseModal {
	private options: EditFlashcardModalOptions;
	private resolvePromise: ((result: EditFlashcardResult) => void) | null = null;
	...
	constructor(app: App, options: EditFlashcardModalOptions) {
		super(app, {
	 ...
	 */
	async openAndWait(): Promise<EditFlashcardResult> {
	...
	onOpen(): void {
		super.onOpen();
		this.contentEl.addClass("episteme-edit-flashcard-modal");
	}
	...
	protected renderBody(container: HTMLElement): void {
		...
		container.createEl("p", {
			text: `Card ID: ${card.id.substring(0, 8)}...`,
			cls: "episteme-modal-info episteme-modal-info-secondary",
		});

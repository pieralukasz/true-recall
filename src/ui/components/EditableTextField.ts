	...
	private renderToolbar(): void {
		...
		for (const btn of buttons) {
			...
			const btnEl = this.toolbar.createEl("button", {
				cls: "ep:px-2 ep:py-1 ep:text-ui-smaller ep:bg-obs-secondary ep:text-obs-normal ep:border ep:border-obs-border ep:rounded-md ep:cursor-pointer ep:hover:bg-obs-modifier-hover ep:hover:border-obs-interactive ep:transition-colors",
				text: btn.label,

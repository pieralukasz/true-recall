...
const PROP_VALUES: { name: string; aliases: string[]; description: string }[] =
	[
...
export function getTokenAtCursor(input: string, cursorPos: number): TokenInfo {
	...
	if (inQuote) {
		...
		if (end < input.length) end++; // include closing quote
...
export function getTokenContext(tokenInfo: TokenInfo): TokenContext {
	...
	const prefixMap: Record<string, TokenContext["type"]> = {
		is: "is",
		prop: "prop",
		note: "note",
		project: "project",
...
export function buildStaticSuggestions(
...
function buildPropSuggestions(partial: string): SearchSuggestion[] {
...
function buildTypeSuggestions(partial: string): SearchSuggestion[] {
...
function buildViaSuggestions(partial: string): SearchSuggestion[] {
...
function buildDateSuggestions(context: TokenContext): SearchSuggestion[] {
	...
	if (prefix !== "added" && prefix !== "reviewed") return [];

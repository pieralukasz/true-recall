...
import { loadDatabase, type DatabaseLike } from "../persistence/sqlite/loader";

const FIELD_SEPARATOR = "\x1f";

// Anki card type/queue constants
const ANKI_QUEUE_SUSPENDED = -1;

// FSRS difficulty -> Anki factor conversion constants
        ...
        for (const card of standalone) {
            ...
            if (includeScheduling) {
                const { type, queue, due, ivl, factor } = this.mapFsrsToAnki(
                    card, collectionCreatedAt, newCardPosition
                );
                ...
                db.run(
                    `INSERT INTO cards (id, nid, did, ord, mod, usn, type, queue, due, ivl, factor, reps, lapses, left, odue, odid, flags, data)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    [
                        cardId, noteId, deckId, ord, nowSecs, -1,
            ...
            if (reversed) {
                ...
                if (includeScheduling) {
                    const { type, queue, due, ivl, factor } = this.mapFsrsToAnki(
                        reversed, collectionCreatedAt, newCardPosition
                    );
                    ...
                    db.run(
                        `INSERT INTO cards (id, nid, did, ord, mod, usn, type, queue, due, ivl, factor, reps, lapses, left, odue, odid, flags, data)
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                        [
                            reversedCardId, noteId, reversedDeckId, 1, nowSecs, -1,
                            type, queue, due, ivl, factor,
                            reversed.reps, reversed.lapses, 0, 0, 0, 0, "",
                        ]
                    );
                } else {
                    db.run(

...
export class FlashcardManager {
	...
	/**
	 * Repair created_at dates for all cards based on source note frontmatter
	 * Used to fix cards that got incorrect dates during V6→V7 migration
	 */
	async repairCardsCreatedDates(): Promise<{ updated: number; skipped: number; errors: string[] }> {
		if (!this.store) {
			throw new Error("Store not initialized");
		}
		...
		if (!sqlStore.getAllSourceNotes || !sqlStore.updateCardsCreatedAtBySourceUid) {
			throw new Error("Store does not support this operation");
		}

		const sourceNotes = sqlStore.getAllSourceNotes();
		console.log('DEBUG: Total source notes to process:', sourceNotes.length);
		let updated = 0;
		let skipped = 0;
		const errors: string[] = [];

		for (const sourceNote of sourceNotes) {
			if (!sourceNote.notePath) {
				skipped++;
				continue;
			}

			const file = this.app.vault.getFileByPath(sourceNote.notePath);
			if (!file) {
				errors.push(`File not found: ${sourceNote.notePath}`);
				continue;
			}

			try {
				const content = await this.app.vault.read(file);
				const createdDate = this.parseCreatedFromFrontmatter(content);
				console.log('DEBUG: Processing', sourceNote.notePath, '-> createdDate:', createdDate);
				...
				const count = sqlStore.updateCardsCreatedAtBySourceUid(
					sourceNote.uid,
					createdDate.getTime()
				);
				updated += count;
			} catch (error) {
				errors.push(`Error reading ${sourceNote.notePath}: ${error}`);
			}
		}
	 ...
	 * Supports formats: "2026-01-05 (Monday)" or "2026-01-05"
	 */
	private parseCreatedFromFrontmatter(content: string): Date | null {
		...
		// Log the raw created line for debugging
		const createdLine = frontmatterMatch[1].split('\n').find(line => line.startsWith('created:'));
		console.log('DEBUG: created line:', JSON.stringify(createdLine));
		...
		console.log('DEBUG: Matched date:', createdMatch[1]);
		return new Date(createdMatch[1]);
	}

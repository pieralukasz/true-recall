...
function getRegionCorner(region: IORegion, corner: ResizeCorner): { x: number; y: number } {
	switch (corner) {
		case "nw":
...
export function IOCanvas({
	...
	useEffect(() => {
		...
		const onPointerMove = (event: PointerEvent) => {
			...
			const point = getPoint(event, stageEl);
			if (!point) return;
		...
		const onPointerUp = () => {
			if (dragState.type === "draw" && draftRegion && draftRegion.w > 0.01 && draftRegion.h > 0.01) {
	...
	const handlePointerDown = (event: PointerEvent) => {
		...
		if ((spacePressed || event.button === 1) && stageEl) {
			setDragState({
				type: "pan",

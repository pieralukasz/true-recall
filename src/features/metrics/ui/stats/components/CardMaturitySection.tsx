...
export function CardMaturitySection({ data }: CardMaturitySectionProps) {
	...
	const total = data.new + data.learning + data.young + data.mature + data.suspended + data.buried;
		...
		(): ChartConfiguration<"doughnut"> | null => {
			...
			return {
				...
				data: {
					...
					datasets: [
						{
							data: [data.new, data.learning, data.young, data.mature, data.suspended, data.buried],
							backgroundColor: [
								CHART_COLORS.green(),
								CHART_COLORS.orange(),
								CHART_COLORS.blue(),
				...
				options: {
					...
					plugins: {
						legend: {
							position: "right",
							labels: { color: CHART_COLORS.normal(), boxWidth: 10, padding: 6, font: { size: 11 } },
						},
					},
				},
			};
	...
	if (total === 0) {
		return (
			<ChartCard title="Card Maturity">
				<p class="ep:text-xs ep:text-obs-muted ep:py-8 ep:text-center">No cards yet</p>
			</ChartCard>
		);
	}
		...
		<ChartCard title="Card Maturity" subtitle={`${maturePercent}% mature — ${total} total cards`}>
			<div class="ep:flex ep:items-center ep:gap-4">
				...
				<div class="ep:grid ep:grid-cols-2 ep:gap-x-4 ep:gap-y-1 ep:text-xs">
					...
					<MaturityRow label="Learning" count={data.learning} color="ep:text-obs-orange" />
					<MaturityRow label="Young" count={data.young} color="ep:text-obs-blue" />
					<MaturityRow label="Mature" count={data.mature} color="ep:text-obs-cyan" />
					<MaturityRow label="Suspended" count={data.suspended} color="ep:text-obs-error" />
					<MaturityRow label="Buried" count={data.buried} color="ep:text-obs-muted" />
				</div>
			</div>
		</ChartCard>
	);
...
function MaturityRow({ label, count, color }: { label: string; count: number; color: string }) {
	return (
